// get the ninja-keys element
const ninja = document.querySelector('ninja-keys');

// add the home and posts menu items
ninja.data = [{
    id: "nav-about",
    title: "about",
    section: "Navigation",
    handler: () => {
      window.location.href = "/";
    },
  },{id: "nav-blog",
          title: "blog",
          description: "",
          section: "Navigation",
          handler: () => {
            window.location.href = "/blog/";
          },
        },{id: "nav-projects",
          title: "projects",
          description: "A growing collection of my projects.",
          section: "Navigation",
          handler: () => {
            window.location.href = "/projects/";
          },
        },{id: "nav-repositories",
          title: "repositories",
          description: "You can find details on my github repos here !",
          section: "Navigation",
          handler: () => {
            window.location.href = "/repositories/";
          },
        },{id: "nav-cv",
          title: "cv",
          description: "Download my CV or view my experience and education below.",
          section: "Navigation",
          handler: () => {
            window.location.href = "/cv/";
          },
        },{id: "post-deep-ml-gpt-2-text-generation",
        
          title: "Deep-ML: GPT-2 Text Generation",
        
        description: "Building GPT-2 text generation from scratch - understanding transformers, embeddings, and layer normalization",
        section: "Posts",
        handler: () => {
          
            window.location.href = "/blog/2026/deepml-gpt2-text-generation/";
          
        },
      },{id: "projects-crowd-evacuation-simulation",
          title: 'Crowd Evacuation Simulation',
          description: "Agent-based modeling of crowd panic situations using numerical schemes (Euler, RK4).",
          section: "Projects",handler: () => {
              window.location.href = "/projects/crowd_evacuation/";
            },},{id: "projects-quantum-hydrodynamic-description-of-plasma",
          title: 'Quantum Hydrodynamic Description of Plasma',
          description: "Validated Wigner-Poisson formulations and implemented a high-performance N-body simulation (PIC approach).",
          section: "Projects",handler: () => {
              window.location.href = "/projects/quantum_plasma/";
            },},{
        id: 'social-email',
        title: 'email',
        section: 'Socials',
        handler: () => {
          window.open("mailto:%61%6E%64%72%65%61%6E%65%65%6E%75@%67%6D%61%69%6C.%63%6F%6D", "_blank");
        },
      },{
        id: 'social-github',
        title: 'GitHub',
        section: 'Socials',
        handler: () => {
          window.open("https://github.com/andreaneenu", "_blank");
        },
      },{
        id: 'social-linkedin',
        title: 'LinkedIn',
        section: 'Socials',
        handler: () => {
          window.open("https://www.linkedin.com/in/andrea-jose-370880255", "_blank");
        },
      },{
        id: 'social-rss',
        title: 'RSS Feed',
        section: 'Socials',
        handler: () => {
          window.open("/feed.xml", "_blank");
        },
      },{
      id: 'light-theme',
      title: 'Change theme to light',
      description: 'Change the theme of the site to Light',
      section: 'Theme',
      handler: () => {
        setThemeSetting("light");
      },
    },
    {
      id: 'dark-theme',
      title: 'Change theme to dark',
      description: 'Change the theme of the site to Dark',
      section: 'Theme',
      handler: () => {
        setThemeSetting("dark");
      },
    },
    {
      id: 'system-theme',
      title: 'Use system default theme',
      description: 'Change the theme of the site to System Default',
      section: 'Theme',
      handler: () => {
        setThemeSetting("system");
      },
    },];
